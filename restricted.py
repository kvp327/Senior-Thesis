...
pd_l1_levels = [1, 2, 3]
pfs_status = [1, 2]
restricted_actions = ["Chemo", "Immuno"]
full_actions = ["Chemo", "Immuno", "ChemoImmuno"]
num_cycles = 10
...
def evaluate_fixed_policy(fixed_action, actions, cost_matrix):
    V = np.zeros((num_cycles + 1, len(states)))
    for t in reversed(range(num_cycles)):
        T = build_transition_matrix(t + 1, fixed_action, fixed_action, actions)
        for s_idx, (tox, pd_l1, pfs) in enumerate(states):
            ...
            if t == num_cycles - 1:
                ...
                total_cost = immediate_cost + np.dot(T[fixed_action][s_idx, :], V[t + 1])
            V[t][s_idx] = total_cost
    return V[0]

# Run
cost_matrix_full = build_cost_matrix(full_actions)
cost_matrix_restricted = build_cost_matrix(restricted_actions)

policy_full, V0_full = value_iteration(
    horizon=num_cycles,
    actions=full_actions,
    cost_matrix=cost_matrix_full,
    build_transition_matrix=build_transition_matrix,
    get_terminal_os_reward=get_terminal_os_reward,
    states=states
)

policy_restricted, V0_restricted = value_iteration(
    horizon=num_cycles,
    actions=restricted_actions,
    cost_matrix=cost_matrix_restricted,
    build_transition_matrix=build_transition_matrix,
    get_terminal_os_reward=get_terminal_os_reward,
    states=states
)

combo_costs = evaluate_fixed_policy("ChemoImmuno", full_actions, cost_matrix_full)
policy_table_data = []
for idx, state in enumerate(states):
    row = [state]
    row += [policy_restricted[t][idx] for t in range(num_cycles)]
    row += [
        round(V0_restricted[idx], 2),
        round(combo_costs[idx], 2),
        round(combo_costs[idx] - V0_restricted[idx], 2),
        round(V0_full[idx] - V0_restricted[idx], 2)
    ]
    policy_table_data.append(row)

columns = ["State"] + [f"Restricted_d*_{i+1}" for i in range(10)] + [
    "Cost_Restricted", "Cost_ComboOnly", "Delta_R_vs_Combo", "Delta_Optimal_vs_Restricted"
]

policy_compare_df = pd.DataFrame(policy_table_data, columns=columns)
policy_compare_df = policy_compare_df.sort_values(by="Delta_R_vs_Combo", ascending=False).reset_index(drop=True)

policy_compare_df.to_csv("restricted_vs_combo_policy_table.csv", index=False)
print("Saved policy comparison table to 'restricted_vs_combo_policy_table.csv'")

# Heatmap of Restricted vs Combo Difference 
heatmap_data = []
for i, (tox, pd_l1, pfs) in enumerate(states):
    delta = combo_costs[i] - V0_restricted[i]
    heatmap_data.append({
        "Toxicity": tox,
        "PDL1": pd_l1,
        "PFS": pfs,
...
custom_blue = "#0077BB"
custom_crimson = "#C93047"
my_bold_cmap = LinearSegmentedColormap.from_list("MyBoldCmap", [custom_blue, custom_crimson])


# Use LaTeX font settings to match your thesis
mpl.rcParams.update({
    "text.usetex": True,
    "font.family": "serif",
...
unique_pfs = sorted(df["PFS"].unique())
fig, axes = plt.subplots(1, len(unique_pfs), figsize=(6 * len(unique_pfs), 4), constrained_layout=True)

if len(unique_pfs) == 1:
    axes = [axes]  # make iterable if only 1 PFS level

for ax, pfs in zip(axes, unique_pfs):
    df_subset = df[df["PFS"] == pfs]
    pivot = df_subset.pivot(index="Toxicity", columns="PDL1", values="Delta")
    ...
    sns.heatmap(
        ...
        linewidths=0.5,
        ax=ax,
        cbar_kws={'label': 'Chemoimmuno - Restricted'} if ax == axes[-1] else None
    )

    ax.set_title(f"Cost Difference for $f$ = {pfs}", fontsize=12, pad=10)
    ax.set_xlabel("PD-L1 State")
    ax.set_ylabel("Toxicity State" if ax == axes[0] else "")
    ...
    if ax == axes[-1]:
        ...
        cbar.set_label("Combo - Restricted", labelpad=20)

# Save as PDF to preserve font quality
os.makedirs("Figures", exist_ok=True)
plt.savefig("Figures/restricted_pfs_combined.pdf", bbox_inches="tight")
plt.show()
