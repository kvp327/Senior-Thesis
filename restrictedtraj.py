...
# Transition Probabilities (without Chemoimmuno)
toxicity_probs = {
    "Chemo": {
...
# 3. Cost Matrix & Value Iteration Functions
def build_cost_matrix():
    ...
    for a in actions:
        for s_idx, (tox, pd_l1, pfs) in enumerate(states):
            ...
            os_val = get_terminal_os_reward(a, pd_l1, pfs)
            normalized_burden = burden / max_burden
            normalized_os = os_val / max_os
    ...
    for t in reversed(range(horizon)):
        for s_idx, s in enumerate(states):
            ...
            for a in actions:
                ...
                if apply_switch_penalty and prev_actions[s_idx] is not None and prev_actions[s_idx] != a:
            ...
            best_idx = np.argmin(action_costs)
            V[t][s_idx] = action_costs[best_idx]
            policy[t][s_idx] = actions[best_idx]
            prev_actions[s_idx] = actions[best_idx]
    return policy, V[0]
...
def simulate_trajectory_and_cost_updated(policy, build_transition_matrix, states, state_index, init_state, horizon=10):
    ...
    prev_action = None
    total_cost = 0.0
    trajectory = []  # (cycle, pre_state, action, post_state)
...
# Run MDP, Simulations, and Build Summary for the Restricted Case
if __name__ == "__main__":
    ...
    for init_state in states:
        ...
        first_actions = [traj[0][2] for traj in trajectories]  
        final_actions = [traj[-1][2] for traj in trajectories]    
    ...
    print(df_summary)
    df_summary.to_csv("final_policy_comparison_restricted.csv", index=False)
    ...
    plt.title("Stochastic Action Trajectories by Initial State", fontsize=14)
    plt.xlabel("Cycle", fontsize=12)
    plt.ylabel("Treatment", fontsize=12)
    plt.yticks([0, 1], labels=["Chemo", "Immuno"])
    plt.xticks(range(1, num_cycles + 1))
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.legend(title="Initial State", title_fontsize=11, fontsize=10)
    plt.tight_layout()
    tikzplotlib.save("stochastic_trajectories_restricted.tex")
    plt.show()
